...
import com.mycarrental.dto.AdminDTO;
import com.mycarrental.dto.UserDTO;
import com.mycarrental.exception.AuthException;
import com.mycarrental.exception.BadRequestException;
import com.mycarrental.exception.ConflictException;
import com.mycarrental.exception.ResourceNotFoundException;
import com.mycarrental.projection.ProjectUser;
import com.mycarrental.repository.RoleRepository;
...
@Service
@AllArgsConstructor //Autowired yerine bunu kullaniyoruz
public class UserService {
    ...
    public void updateUser(Long id, UserDTO userDTO) throws BadRequestException {
        ...
        if (userDetails.get().getBuiltIn()) {
            throw new BadRequestException("You dont have permission to update user info!(UserService)");
        }

        if (emailExists && !userDTO.getEmail().equals(userDetails.get().getEmail())) {
            throw new ConflictException("Error: Email is already in use!(UserService)");
        }
